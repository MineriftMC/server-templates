...
# seed the data dir with the premade void server files, first run only
SEED_MARKER="${DATA_DIR}/.void-files-seeded"
if [[ ! -f "${SEED_MARKER}" ]]; then
  if [[ -d /opt/minerift/files ]] && compgen -G "/opt/minerift/files/*" >/dev/null; then
    cp -rf /opt/minerift/files/. "${DATA_DIR}/"
  fi
  touch "${SEED_MARKER}"
fi
